class RestrictedDict(dict):
	...
	def __repr__(self):
		...
		# return 'RestrictedDict(%s, %s)' % (self._allowed_keys.__repr__(), super(RestrictedDict, self).__repr__())
		return 'RestrictedDict(%s)' % (super(RestrictedDict, self).__repr__())
...
class RestrictedDictKeyValuetypes(RestrictedDict):
	"""
	Stores the properties of an object. It's a dictionary that's
	restricted to a tuple of allowed keys and values. Any attempt to set an invalid
	key raises an error.
	"""

	def __init__(self, allowed_keys_valuetypes,
							 seq=(), ignore_extras=False, **kwargs
							 ):
		"""
		Initializes the class instance. The allowed_keys tuple is
		required, and it cannot be changed later.
		...
		"""
		# normalize arguments to a (key, value) iterable
		self._allowed_type_for_key={i[0]:i[1] for i in allowed_keys_valuetypes}
		self._allowed_keys_valuetypes = allowed_keys_valuetypes
		self._allowed_keys=[i[0] for i in allowed_keys_valuetypes]
		...
		# scan the items keeping track of the keys' order
		for k, v in seq:
			self.__setitem__(k, v, ignore_extras)

	def __setitem__(self, key, value, allow_none=True):
		"""Checks if the key is allowed before setting the value"""
		if isinstance(value, self._allowed_type_for_key[key]):
			super(RestrictedDict, self).__setitem__(key, value)
		elif value is None:
			super(RestrictedDict, self).__setitem__(key, value)
		else:
			raise TypeError("Type %s is not allowed for key %s" % (type(value).__name__, key))

	def __repr__(self):
		"""Representation of the RestrictedDict"""
		return 'RestrictedDictKeyValuetypes(%s, %s)' % (self._allowed_type_for_key.__repr__(), super(RestrictedDict, self).__repr__())
